...
BOOL UpdatingForm_OnInitDialog(HWND hWnd, WPARAM wParam, LPARAM lParam) {
	...
	while (i < count) {
		...
		item.iSubItem = 1;
		item.pszText = wordBook->englishWords[i].word;
		SendMessage(GetDlgItem(hWnd, IDC_LIST_ENGLISHWORDS), LVM_SETITEMTEXT, (WPARAM)i, (LPARAM)&item);

		item.iSubItem = 2;
		item.pszText = wordBook->englishWords[i].wordClass;
		SendMessage(GetDlgItem(hWnd, IDC_LIST_ENGLISHWORDS), LVM_SETITEMTEXT, (WPARAM)i, (LPARAM)&item);

		item.iSubItem = 3;
		item.pszText = wordBook->englishWords[i].meaning;
		SendMessage(GetDlgItem(hWnd, IDC_LIST_ENGLISHWORDS), LVM_SETITEMTEXT, (WPARAM)i, (LPARAM)&item);

		item.iSubItem = 4;
		item.pszText = wordBook->englishWords[i].example;
		SendMessage(GetDlgItem(hWnd, IDC_LIST_ENGLISHWORDS), LVM_SETITEMTEXT, (WPARAM)i, (LPARAM)&item);
		i++;
	}
