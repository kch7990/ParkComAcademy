...
Long Load(BusinessCardHolder *businessCardHolder) {
	...
	FILE *personalsFile;
	FILE *companiesFile;
	size_t personalsSize;
	size_t companiesSize;

	personalsFile = fopen("Personals.dat", "rb");
	companiesFile = fopen("Companies.dat", "rb");
	if (personalsFile != NULL && companiesFile != NULL) {
		personalsSize = fread(&index, sizeof(Long), 1, personalsFile);
		personalsSize = fread(&personal, sizeof(Personal), 1, personalsFile);
		while (!feof(personalsFile) && personalsSize != 0) {
			fseek(companiesFile, 0L, SEEK_SET);
			i = 1;
			companiesSize = fread(&company, sizeof(Company), 1, companiesFile);
			while (!feof(companiesFile) && companiesSize != 0 && i < index) {
				i++;
				companiesSize = fread(&company, sizeof(Company), 1, companiesFile);
			}
